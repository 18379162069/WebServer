...
<div class="wrapper row-offcanvas row-offcanvas-left">
    ...
    <aside class="right-side">
        ...
        <section class="content">
            <div class="">
                <input type="hidden" id="tmpId" value="${id}">
                <script id="editor" type="text/plain" style="width:100%;height:500px;">
                </script>
                <script>
                    var ue = UE.getEditor('editor');
                </script>
                <input type="button" onclick="submit()" style="width: 100% !important; margin-top: 1%" class="btn btn-primary" value="发布"/>
            </div>
...
<script type="text/javascript">
    ...
    function submit(){
        var id = $("#tmpId").val();
        var news = ue.getContent();


        $.ajax({
            type: 'POST',
            url: "<%=basePath %>admin/adminPubNews",
            data: {"news": news,"id":id},
            dataType: "json",
            async: false,
            success: function () {
                window.location.href="<%=basePath%>admin/newsList"
            }
        });
    }
