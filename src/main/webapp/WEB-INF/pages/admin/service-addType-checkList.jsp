...
<script>
    $(document).ready(function(){
        if(window.location!="<%=basePath%>admin/serviceTypeCheckList"){
            window.location.href="<%=basePath%>admin/serviceTypeCheckList";
        }
    })
</script>
                                ...
                                <div class="col-md-12 col-sm-12">
                                    ...
                                    <div class="table-responsive">
                                        ...
                                        <table class="table table-mailbox">
                                            ...
                                            <c:if test="${serviceTypeCheckList!=null}">
                                                <c:forEach items="${serviceTypeCheckList}" varStatus="state" var="obj">
                                                    <tr class="unread">
                                                        <input type="hidden" value="${obj.id}"/>
                                                        <td class="small-col"><input type="hidden"></td>
                                                        <td class="name"><a href="<%=basePath%>admin/courseCheckInfo?id=${obj.id}">${obj.type1name}</a></td>
                                                        <td class="subject">${obj.type2name}</td>
                                                        <td class="time"><a href="<%=basePath%>admin/argServiceTypeCheck?id=${obj.id}&agree=yes"><input type="button" class="btn-small btn-primary" value="通过"></a></td>
                                                        <td class="time"><a href="<%=basePath%>admin/argServiceTypeCheck?id=${obj.id}&agree=no"><input type="button" class="btn-small btn-danger" value="不通过"></a></td>
                                                    </tr>
                                                </c:forEach>
                                            </c:if>
                                        </table>
