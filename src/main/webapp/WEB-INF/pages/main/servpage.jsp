...
<div class="container" style="margin-top: 5%">
    <div class="row">
        ...
        <div class="col-md-8 serviceList">
            <div class="row">
                <div class="" style="border: solid 1px rgb(220,220,220);height: 40px;margin-right:9%">
                </div>
            </div>

            <div class="row row_tran_class" style="margin-top: 30px;height: 30%">
                <c:if test="${serviceList!=null}">
                    <c:forEach items="${serviceList}" var="obj" varStatus="status">
                        <div class="col-md-3"
                             style="border: solid 1px rgb(220,220,220);border-radius: 2px;text-align: left;padding: 0;margin-bottom: 10px;">
                            <a href="<%=basePath%>web/getServiceInfo?serviceId=${obj.id}"><img
                                    src="<%=basePath%>web/getPic/serviceImg/${obj.servicepic}" alt=""
                                    style="width: 100%;height: 40%!important;"></a>
                            <h4 style="display: inline-block;margin-left: 10px"
                                cid="${status.step}">${obj.servicename}</h4>
                        </div>
            ...
            <div style="text-align: center">
                <ul class="pagination">
                    <c:if test="${pageNum>1}">
                        <li>
                            <a href="<%=basePath%>web/getTyp2ServieList?type2=${sessionScope.type2}&pageNum=${pageNum-1}">&laquo;</a>
                        </li>
                    </c:if>
                    <c:if test="${page.pageCount<=10}">
                        <c:forEach begin="1" end="${page.pageCount}" step="1" var="i">
                            <li><a href="<%=basePath%>web/getTyp2ServieList?type2=${sessionScope.type2}&pageNum=${i}">${i}</a>
                            </li>
                        </c:forEach>
                    </c:if>
                    <c:if test="${page.pageCount>10}">
                        <c:forEach begin="${page.preStart}" end="${page.preEnd}" step="1" var="i">
                            <li><a href="<%=basePath%>web/getTyp2ServieList?type2=${sessionScope.type2}&pageNum=${i}">${i}</a></li>
                        </c:forEach>
                        <li><a href="">...</a></li>
...
<script type="text/javascript">
    ...
    function show2List(data) {
        ...
        $.each(data, function (idx, obj) {
            $(".type2Each").append("<p style='border-top: solid 1px rgb(233,233,233)'>"
                    + "<a href='<%=basePath%>web/getTyp2ServieList?type2=" + obj + "'>" + obj + "</a></p>");
            $(".type2List").css("display", "block");
        });
    }
    function get2List(index) {
        $.ajax({
            type: "GET",
            url: "<%=basePath%>web/getType2For1Service",
            data: {"index": index},
            dataType: "json",
            async: false,
